...
import {
  ...
  fetchCvFromFirebase,
  EMPTY_CV_DATA,
} from './services/firebaseCvApi';

function App() {
  // Firebase Auth user state
  const [user, setUser] = useState<User | null>(null);

  // Initialize with Spanish CV data (from localStorage if present, otherwise EMPTY_CV_DATA)
  ...
  const handleSaveToCloud = async () => {
    await saveCvToFirebase(activeData);
  };

  const handleFetchFromCloud = async () => {
    const cloudData = await fetchCvFromFirebase();
    if (currentLang === 'es') {
      setCvDataEs(cloudData);
    } else {
      setCvDataEn(cloudData);
    }
  };
              ...
              <div className="modal-dialog modal-dialog-centered" role="document" onClick={(e) => e.stopPropagation()}>
                <div className="modal-content border border-secondary border-opacity-15 bg-dark text-white rounded-3 shadow-lg">
                  ...
                  <div className="modal-body p-4 text-start">
                    <div
                      className="alert alert-info border-secondary border-opacity-15 bg-dark bg-opacity-25 py-2 px-3 rounded text-muted mb-3"
                      style={{ fontSize: '0.85rem' }}
                    >
                      <strong>¿Cómo obtener tu clave?</strong> Para traducir tu currículum al inglés con IA, necesitas una
                      API Key de Gemini.
                      <br />
                    ...
                    <div className="form-group mb-2">
                      <label htmlFor="modal-gemini-key" className="form-label mb-1">
                        Gemini API Key
                      </label>
                      ...
                      {testResult === 'success' && (
                        <div
                          className="text-success mt-2 fw-medium d-flex align-items-center gap-1"
                      ...
                      {testResult === 'failure' && (
                        <div
                          className="text-danger mt-2 fw-medium d-flex align-items-center gap-1"
                          style={{ fontSize: '0.82rem' }}
                        >
                          <span className="material-symbols-outlined" style={{ fontSize: '16px' }}>
                            close
                  ...
                  <div className="modal-footer border-top border-secondary border-opacity-15 p-3 d-flex justify-content-end gap-2">
                    ...
                    <button type="button" className="btn btn-primary px-4" onClick={handleSaveApiKey}>
