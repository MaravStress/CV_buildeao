...
export const Editor: React.FC<EditorProps> = ({
  ...
  const loadFirebaseCvData = async () => {
    setIsLoadingFromDb(true);
    try {
      await onFetchFromCloud();
    } catch (err: any) {
      alert(err.message || 'Error al obtener la información desde Firebase BD.');
    } finally {
  ...
  const getModalHeaderTitle = (type: ModalType) => {
    switch (type) {
      ...
      case 'settings': return 'Configuración de Página y Firebase';
      ...
      case 'settings':
        return (
          <SettingsForm
            ...
            onApplyCvData={onChange}
            user={user}
            onLoginGoogle={onLoginGoogle}
            onLogoutGoogle={onLogoutGoogle}
            onSaveToCloud={onSaveToCloud}
            onFetchFromCloud={onFetchFromCloud}
          />
        );
      case 'ai-import':
        return (
  ...
  const sectionNavs = [
    ...
    },
    {
      ...
      title: 'Configuración & Admin Cloud',
      desc: user ? `Admin: ${user.email}` : `Tipografía: ${data.settings.fontFamily.split(',')[0]}`,
      sub: user ? '✓ Conectado a Firebase DB' : 'Configura fuente, márgenes o inicia sesión Firebase',
  ...
  return (
    <div className="editor-panel col-12 col-lg-4 col-xl-3 h-lg-100 d-flex flex-column border-end border-secondary border-opacity-10 overflow-lg-hidden no-print">
      ...
      <div className="editor-topbar p-3 border-bottom d-flex justify-content-between align-items-center bg-dark bg-opacity-25">
        ...
        <div className="topbar-actions d-flex gap-2">
          <button
            ...
            onClick={loadFirebaseCvData}
            disabled={isLoadingFromDb}
            title="Cargar CV oficial de Eliam Paredes desde Firebase BD (portfolio/bdData)"
          >
            {isLoadingFromDb ? (
              <span className="spinner-border spinner-border-sm me-1" role="status" aria-hidden="true"></span>
            ) : (
              <span className="material-symbols-outlined me-1" style={{ fontSize: '14px' }}>cloud_download</span>
            )}
            <span className="d-none d-xl-inline">Eliam (Firebase)</span>
          </button>
          <button
      ...
      <div className="editor-scrollable flex-grow-1 overflow-lg-auto p-3 d-flex flex-column gap-2">
        <div className="mb-2">
          <span className="text-muted uppercase fw-semibold" style={{ fontSize: '0.75rem', letterSpacing: '0.5px' }}>
        ...
        {sectionNavs.map((nav) => (
          <button
            key={nav.id}
            className={`section-nav-card p-3 text-start border border-secondary border-opacity-10 rounded-3 bg-dark bg-opacity-25 transition-all d-flex align-items-center gap-3 w-100 ${
              hoveredSection === nav.id ? 'active-nav-card' : ''
            }`}
          ...
          >
            ...
            <div className="nav-card-details flex-grow-1 overflow-hidden">
              ...
              <p className="nav-card-desc m-0 text-truncate text-muted mt-1" style={{ fontSize: '0.78rem' }}>
                {nav.desc}
              </p>
              <p className="nav-card-sub m-0 text-truncate text-secondary opacity-50 mt-0.5" style={{ fontSize: '0.7rem' }}>
                {nav.sub}
              </p>
            </div>
          </button>
      ...
      <div className="editor-footer p-3 border-top d-flex flex-column gap-2 bg-dark bg-opacity-25">
        <div className="data-transfer-row d-flex gap-2">
          <button
            className="btn btn-outline flex-btn-sm w-50 justify-content-center d-flex align-items-center gap-1"
            onClick={exportJSON}
          >
